<?php
$id = $_GET['id']; 
$sql_user = "SELECT * From `tbl_nhanvien` where `ID`='$id'"; 
$query_user = mysqli_query($con, $sql_user);
$row = mysqli_fetch_array($query_user);

$check_pass = "SELECT * from `tbl_user` where  `name`=" . '\'' . $row['Ho'] ." ".$row['Ten'].'\''; 

$check_query = mysqli_query($con, $check_pass); 
$check_row_sql=mysqli_num_rows($check_query);
echo $check_row_sql;die;
...
if (isset($_POST['btn_addstaff'])) {
    ...
    if ($password == "" || $opassword == "" || $rpassword == "") {
        $_SESSION['err'] = 'Mời nhập mật khẩu';
        ...
        if ($rpassword != $password) {
            ...
            $change_sql = "UPDATE `tbl_user` SET `password`='" . md5($password) . "' where   `name`=" . '\'' . $row['Ho'] ." ".$row['Ten'].'\'';
            ...
            if ($query == true) {
                ...
                // 
                $bc_sql = "INSERT INTO `tbl_baocom`(`username`, `HoTen`, `status`) Values ('$username','$name','0')";
